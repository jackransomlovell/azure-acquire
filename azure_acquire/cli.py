...
@click.option(
    "--recording-length", "-t", type=float, default=30, help="recording time (minutes)"
)
...
@click.option(
    "--preview", default=True, type=bool, help="show frame preview during recording"
)
@click.option(
    "--display-time", default=True, type=bool, help="show time during the recording"
)
@click.option(
    "--sync-mode", default="standalone", type=str, help="sync mode for external trigger"
)
def record(
    base_dir,
    subject_name,
